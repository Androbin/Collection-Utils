...
public final class CharacterCollectionUtil
{
	...
	{
		IntStream.range( 0, array.length ).parallel().forEach( i -> array[ i ] = function.applyAsChar( i ) );
		return array;
	}
	...
	{
		return array == null || array.length == 0 ? (char) Character.UNASSIGNED : array[ ( random == null ? ThreadLocalRandom.current() : random ).nextInt( array.length ) ];
	}
}